...
class Login extends CI_Controller {
	...
	public function index() {
                ...
                if((strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') !== false) && empty($wxinfo)){
                    ...
                    $userinfo = $this->student_model->get_row("unionid = '".$wxinfo['unionid']."' and unionid<>'' ");
                    if(!empty($userinfo)){
                    ...
                    if (!empty($act)) {
                        ...
                        $userinfo = $this->student_model->get_row(array('mobile'=>$mobile));
	...
	public function register1(){
		...
		if(!empty($act)){
			...
			$userinfo = $this->student_model->get_row(array('mobile'=>$user['mobile']));
                        if(empty($company)){
                                $res['msg']='公司编码错误';
